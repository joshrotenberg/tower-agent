...
check:
    ...
    cargo package -p tower-agent -p tower-agent-claude -p tower-agent-codex -p tower-agent-workflow -p tower-agent-plan -p tower-agent-mcp --allow-dirty --locked --no-verify
    just check-examples
